...
for group = 1:ngroups  % load in by group because 'planning only' and 'control' groups have different total no. trials
    ...
    switch group
        ...
        case 2
            load NoImageryFixationTransfer_combined;
    end
...
for group = 1:ngroups
    ...
    inclchannelim = ((round(D.HomeAngle)==-180) | (round(D.HomeAngle==0)));
    basechannels =  find(((field(:,S.FIELD_CHANNEL)==1) & inclchannel) & (D.ContextType==0 & basephase));
    ind = find(((field(:,S.FIELD_CHANNEL)==1) & inclchannel) & (D.ContextType==0));
    indexcltrans = find(((field(:,S.FIELD_CHANNEL)==1) & inclchannel) & ((D.ContextType==0) & ~transferphase));
    indexp = find((field(:,S.FIELD_CHANNEL)==1) & (exposurephase & inclchannel));
    indtrans = find(((field(:,S.FIELD_CHANNEL)==1) & (D.ContextType==0)) & (transferphase & inclchannel));
    indtransim = find(((field(:,S.FIELD_CHANNEL)==1) & (D.ContextType==2)) & (transferphase & inclchannelim));
    
    incl0channel = (round(D.HomeAngle==0) & (D.ContextType==0));
    basechannels0 =  find(((field(:,S.FIELD_CHANNEL)==1) & incl0channel) & (D.ContextType==0 & basephase));
    ...
    h2=shadeplot(linspace(pre+exp+1,exp+trans+pre,length(adaptblocktransim_mean)), adaptblocktransim_mean, adaptblocktransim_se,'-',colours(group+1,:),0.3); hold all;  % exposure
    ...
    % execution trials - first 4 blocks of transfer phase
    finaladapt_mean = mean(adaptationsubjfinalblocktrans);
    finaladapt_se = std(adaptationsubjfinalblocktrans) ./ sqrt(nsubj);
    errorbar(pre+exp+trans+post+9+group*3, finaladapt_mean,finaladapt_se,'k'); hold on;
    plot(pre+exp+trans+post+9+group*3, finaladapt_mean, 'o','MarkerSize',7,'MarkerEdgeColor',colours(group,:), 'MarkerFaceColor',colours(group,:)); hold on;
    ...
    plot(pre+exp+post+trans+6+group*3, finaladapt_mean, 'o','MarkerSize',7,'MarkerEdgeColor',colours(group+1,:), 'MarkerFaceColor',colours(group+1,:)); hold on;
